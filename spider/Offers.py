...
def getImg(soup,store_id):
    try:
        imagename = "/Users/liuzhiguo/Sites/Br.Colpon/public/images/Store-"+str(store_id)+".png"
...
def ifImg(store_id):
    filename = '/Users/liuzhiguo/Sites/Br.Colpon/public/images/Store-'+str(store_id)+'.png'
    return os.path.exists(filename)
def main():
    #SQLs
    sqlselectstores = "SELECT * FROM stores"
    sqlselectoffers = "SELECT %s FROM offers WHERE store_id = %s"
    sqlinsertoffers = "INSERT INTO offers (store_id,type,name,description,link,code,status,confirm_at,end,created_at,updated_at) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    sqlupdateoffers = "UPDATE offers set status = %s, link = %s, confirm_at = %s, end = %s, updated_at = %s where id = %s"
    ...
    for store in stores:
        store_id = store[0]
        link = store[4]
        url = 'https://www.meliuz.com.br/desconto/' + store[5]
        ...
        for offer in offers:
            ...
            confirm_at = datetime.datetime.now() - datetime.timedelta(days = int(random.uniform(0, 3)), hours = int(random.uniform(1, 23)), minutes = int(random.uniform(1, 59)), seconds = int(random.uniform(1, 59)))
            end = datetime.datetime.now() + datetime.timedelta(days = int(random.uniform(5, 30)), hours = int(random.uniform(1, 23)), minutes = int(random.uniform(1, 59)), seconds = int(random.uniform(1, 59)))
            n = 0
            offer_id = 0
            ...
            if n == 0 and name != "" and description != "":
                #Insert Offers
                cursor.execute(sqlinsertoffers,(store_id,codetype,name,description,link,code,1,confirm_at,end,now,now))
                logging.info("Insert Offers")
            else:
                #Update Offers Status = 1
                cursor.execute(sqlupdateoffers,(1,link,confirm_at,end,now,offer_id))
                logging.info("Update Offers Status = 1")
            if description == "" and code == "" and n == 0:
                description = 'As melhores ofertas e promoções'
                #Insert Offers
                cursor.execute(sqlinsertoffers,(store_id,"D",name,description,link,"",1,confirm_at,end,now,now))
