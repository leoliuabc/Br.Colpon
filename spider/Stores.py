...
def googleGetLink(keyword):
    try:
        ...
        soup = BeautifulSoup(str(html.content), "html.parser")
        link = soup.find('cite',"_Rm").get_text()
        return link
    except:
...
def main():
    ...
    sqlupdate = "UPDATE stores set link = %s, updated_at = %s where id = %s"
    sqlinsert = "INSERT INTO stores (initials,name,description,link,titleslug,created_at,updated_at) values (%s,%s,%s,%s,%s,%s,%s)"
    for href in hrefs:
        #Open SQL
        ...
        store_id = 0
        for store in stores:
            if store[5] == titleslug[0]:
                slug = 1
                if store[4] =="":
                    store_id = store[0]
        #If Need Google Search
        if slug == 0 or store_id != 0:
            link = googleGetLink(name)
            if 'https://' not in link:
                link = 'http://' + link
        if slug == 0:
            #Insert Stores
            cursor.execute(sqlinsert,(initials,name,description,link,titleslug,now,now))
            logging.info("Insert Store name: %s" % name)
        if store_id != 0:
            #Update Stores
            cursor.execute(sqlupdate,(link,now,store_id))
